...
class SiameseCSNN(object):
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('attention') as scope1:
            ...
            # 维度 (?,512,200)  (?,512,15)
            atted_q_pos, atted_c_pos = self.atten_xydir_wrapper(lstm_q, lstm_c_pos, 'xydiratt')

            scope1.reuse_variables()  # 复制参数
            # 维度 (?,512,200)  (?,512,15)
            atted_q_neg, atted_c_neg = self.atten_xydir_wrapper(lstm_q, lstm_c_neg, 'xydiratt')
    ...
    def add_loss_op(self, p_sim, n_sim):
        """
        损失节点
        """
