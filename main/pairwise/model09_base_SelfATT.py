...
class SiameseCSNN(object):
    ...
    def selfAttentive(self,H, scope, hidden_size, d_a =256, r = 1):
        # shape(H) = n * 2u
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('self-attention') as scope1:
            # LSTM (?, 512)
            selfed_c_pos = self.selfAttentive(lstm_c_pos, "code", self.hidden_size, r=1)

            # LSTM (?, 512)
            selfed_q = self.selfAttentive(lstm_q, "query", self.hidden_size, r=1)

            scope1.reuse_variables()  # 复制参数
            # LSTM (?, 512)
            selfed_c_neg = self.selfAttentive(lstm_c_neg, "code", self.hidden_size, r=1)


        # todo 2：COS式

        q_pos_cosine = tf.reduce_sum(tf.multiply(tf.nn.l2_normalize(selfed_q, dim=1),
                                                           tf.nn.l2_normalize(selfed_c_pos, dim=1)), axis=1)

        q_neg_cosine = tf.reduce_sum(tf.multiply(tf.nn.l2_normalize(selfed_q, dim=1),
                                                           tf.nn.l2_normalize(selfed_c_neg, dim=1)), axis=1)
