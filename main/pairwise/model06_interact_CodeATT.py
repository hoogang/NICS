...
print('model06_interact_CodeATT')
...
class SiameseCSNN(object):
    ...
    def atten_pool_wrapper(self, input_q, input_c,scope):

        # (?,20,512) (?,200,512)

        output_q = self.max_pooling(input_q)       # (?,512)

        reshape_q = tf.expand_dims(output_q,1)     # (?,1,512)
        reshape_q = tf.tile(reshape_q, [1, self.code_len, 1])  # (?,200,512)
        reshape_q = tf.reshape(reshape_q, [-1, 2*self.hidden_size])  # (?*200,512)

        reshape_c= tf.reshape(input_c, [-1, 2*self.hidden_size])     # (?*200,512)

        Wq = tf.get_variable('Wq' + scope, shape=[2 * self.hidden_size, self.layer_size],
                             initializer=tf.truncated_normal_initializer(stddev=0.1))

        Wc = tf.get_variable('Wc'+scope, shape=[2*self.hidden_size,self.layer_size],
                            initializer=tf.truncated_normal_initializer(stddev=0.1))

        W = tf.get_variable('W'+scope, shape=[self.layer_size,1],
                             initializer=tf.truncated_normal_initializer(stddev=0.1))

        #  代码在查询上的注意力 （？*200,512）*(512,300) = (?*200,300)
        Mqc = tf.tanh(tf.add(tf.matmul(reshape_q, Wq),tf.matmul(reshape_c, Wc)))

        #  (?*200,300) *(300,1) =(?*200,1) ----(?,200)
        S = tf.reshape(tf.matmul(Mqc, W), [-1, self.code_len])  # (?,200)

        #  注意力机制的值   (?,200,200)
        att = tf.matrix_diag(tf.nn.softmax(S))

        #  查询作用在代码上权重  (?,200,200) * (？,200,512)=（？,200,512)
        atten_q2c = tf.matmul(att, input_c)

        #  维度 (?,512)
        output_c = self.max_pooling(atten_q2c)
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('attention') as scope1:
            # 维度 (?,512,)  (?,512)
            q_pos_atted, c_pos_atted = self.atten_pool_wrapper(lstm_q, lstm_c_pos, 'poolatt')

            scope1.reuse_variables()  # 复制参数
            # 维度 (?,512)  (?,512)
            q_neg_atted, c_neg_atted = self.atten_pool_wrapper(lstm_q, lstm_c_neg, 'poolatt')
