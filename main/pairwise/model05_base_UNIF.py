...
class SiameseCSNN(object):
    ...
    def attweight(self,input_embeds,scope):

        dim = input_embeds.get_shape().as_list()[2]

        c= tf.get_variable('c'+scope, shape=[dim,1],
                             initializer=tf.truncated_normal_initializer(stddev=0.1))
        # (?,200,300) * (300,1) =(?,200)
        att= tf.nn.softmax(tf.squeeze(tf.einsum('abc,cd->abd',input_embeds,c),[2]))

        # (?,200,300) * (?,200)
        att_out = tf.einsum('abc,ab->ac',input_embeds,att)
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('attweight') as scope0:
            ...
            c_pos_tanh = tf.nn.tanh(self.attweight(c_pos_embed,'pos_code'))

            q_tanh = tf.nn.tanh(tf.reduce_mean(q_embed, axis=1, keep_dims=False))

            c_neg_tanh = tf.nn.tanh(self.attweight(c_neg_embed, 'neg_code'))
