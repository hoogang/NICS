...
class SiameseCSNN(object):
    ...
    def build(self, embeddings):
        ...
        with tf.variable_scope('AP') :
            ...
            # 中间矩阵 (512,512)
            U=tf.Variable(tf.truncated_normal(shape=[2*self.hidden_size,2*self.hidden_size],dtype=tf.float32,stddev=0.01),name='U')

            # 交互结果 (?,15,200)   (?,15,512) * (?,512,512) * (?,512,200)
            G_pos=tf.matmul(tf.matmul(lstm_q, tf.tile(tf.expand_dims(U, 0), [self.batch_size, 1, 1])),lstm_c_pos,transpose_b=True)

            # 维度 (?, 15)
            delta_q_pos = tf.nn.softmax(tf.reduce_max(G_pos, 2,keep_dims=False))
            # 维度 (?, 200)
            delta_c_pos = tf.nn.softmax(tf.reduce_max(G_pos, 1,keep_dims=False))

            # 维度 (?,512)  (?,512,15)  * (?,15,1)
            feat_q_pos = tf.squeeze(tf.matmul(lstm_q, tf.expand_dims(delta_q_pos, -1), transpose_a=True))
            # 维度 (?,512)  (?,512,200) * (?,200,1)
            feat_c_pos = tf.squeeze(tf.matmul(lstm_c_pos, tf.expand_dims(delta_c_pos, -1), transpose_a=True))

            # # 维度 (?,512)   (?,512,15)  * (?,15,1)
            # feat_q_pos = self.max_pooling(tf.matmul(lstm_q, tf.expand_dims(delta_q_pos, -1), transpose_a=True))
            # # 维度 (?,512)   (?,512,200) * (?,200,1)
            # feat_c_pos = self.max_pooling(tf.matmul(lstm_c_pos, tf.expand_dims(delta_c_pos, -1), transpose_a=True))

            #########################################################################################################

            # 交互结果 (?,15,200)  (?,15,512) *(?,512,512) * (?,512,200)
            G_neg=tf.matmul(tf.matmul(lstm_q,tf.tile(tf.expand_dims(U,0),[self.batch_size,1,1])),lstm_c_neg,transpose_b=True)

            # (?, 15)
            delta_q_neg = tf.nn.softmax(tf.reduce_max(G_neg, 2,keep_dims=False))
            # (?, 200)
            delta_c_neg = tf.nn.softmax(tf.reduce_max(G_neg, 1,keep_dims=False))

            # 维度 (?,512)     (?,512,15)  * (?,15,1)
            feat_q_neg = tf.squeeze(tf.matmul(lstm_q,tf.expand_dims(delta_q_neg, -1),transpose_a=True))
            # 维度 (?,512)     (?,512,200) * (?,200,1)
            feat_c_neg = tf.squeeze(tf.matmul(lstm_c_neg,tf.expand_dims(delta_c_neg, -1),transpose_a=True))

            # # 维度 (?,512)   (?,512,15)  * (?,15,1)
            # feat_q_neg = self.max_pooling(tf.matmul(lstm_q, tf.expand_dims(delta_q_neg, -1), transpose_a=True))
            # # 维度 (?,512)   (?,512,200) * (?,200,1)
            # feat_c_neg = self.max_pooling(tf.matmul(lstm_c_neg, tf.expand_dims(delta_c_neg, -1), transpose_a=True))


        # todo 2：COS式

        q_pos_cosine = tf.reduce_sum(tf.multiply(tf.nn.l2_normalize(feat_q_pos, dim=1),
                                                           tf.nn.l2_normalize(feat_c_pos, dim=1)), axis=1)

        q_neg_cosine = tf.reduce_sum(tf.multiply(tf.nn.l2_normalize(feat_q_neg, dim=1),
                                                           tf.nn.l2_normalize(feat_c_neg, dim=1)), axis=1)
