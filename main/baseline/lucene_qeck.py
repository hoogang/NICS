...
def rank_code_score(pred_path,file_path,lang_type,pair_m,word_n):
    ...
    # 质量打分归一化
    upvote_scores = lang_data['nl_favorite'].values
    ...
    for id in range(pred_length):
        ...
        #打分值整合,加权
        pairs_scores = 0.7*content_scores+0.3*upvote_scores
        #创建 健-值对字典
        pairs_value={}
        ...
        #print('Bm25增加的词:\t', expand_words)
        #加入到query查询中
        expand_query = query_words + expand_words
        # 按索引提取code
    ...
    # 保存数据
    result_data.to_csv(pred_path + '%s_parse_pred_res3.csv' % lang_type, sep='|', encoding='utf-8')  # 保存行名
    print('预测数据处理完毕！')
